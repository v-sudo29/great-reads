...
const Profile = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (
      ...
      updateNameRef.current?.value.replace(/\s/g, '').length
    ) {
    ...
    return (
      <div className="flex flex-col items-center p-2">
        <h1 className="page_heading">
          <b>{session.user ? `${session.user.firstName}'s` : 'my'}</b> profile
        </h1>
        ...
        <div className="flex flex-col gap-4 mt-10">
          ...
          <div>
            ...
            />
            <button className="general_blue_button mt-2" onClick={handleUpdate}>
              Update
            </button>
          </div>
