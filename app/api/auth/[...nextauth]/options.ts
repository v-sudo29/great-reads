...
const authOptions: AuthOptions = {
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials, req): Promise<any> {
        ...
        const imageUrl = transformedUser.imageUrl
  ...
  callbacks: {
    async jwt({ token, user, session, trigger }) {
      ...
      // If user updates FRIENDS, update session and token and database
      ...
      if (user?.id && /^\d+$/.test(user.id)) {
        ...
        if (foundUser?._id) {
          ...
          token.imageUrl = foundUser.imageUrl
        }
      }
    ...
    async signIn({ profile, user }) {
      ...
      try {
        ...
        user.imageUrl = userExists!.imageUrl
