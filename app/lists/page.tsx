...
const Lists = () => {
  ...
  const handleCreateList = async () => {
    if (
      ...
      newListRef.current?.value.replace(/\s/g, '').length
    ) {
  ...
  const handleDeleteList = async (
    ...
    const headingElement = buttonElement.parentElement
      ?.previousSibling as HTMLHeadingElement
    ...
    if (session) {
      const copyList = Object.assign(
        {},
        session.user.lists as Record<string, IBook[]>
      )
  ...
  const handleClickEdit = async (
    ...
    setShowModal(true)
    const buttonElement = e.target as HTMLButtonElement
    const headingElement = (buttonElement.parentElement as HTMLDivElement)
      .previousSibling as HTMLHeadingElement
  ...
  const handleUpdateListName = async () => {
    if (
      updatedListNameRef &&
      updatedListNameRef.current &&
      currentListNameRef.current
    ) {
      const copyLists = Object.assign(
        {},
        session?.user.lists as Record<string, IBook[]>
      )
      copyLists[updatedListNameRef.current.value] =
        copyLists[currentListNameRef.current]
      ...
      <div className="flex flex-col gap-3 my-5">
        ...
        {success && (
          <h1 className="text-green-500">
            {newListRef.current && newListRef.current.value} list has been
            created!
          </h1>
        )}
        ...
        <div className="self-center">
          <button onClick={handleCreateList} className="general_button">
            Create
          </button>
        </div>
      </div>
